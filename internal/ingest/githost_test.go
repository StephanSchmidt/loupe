...
func (f *fakeGitHost) ListCommits(_ context.Context, repo githost.RepoRef, since time.Time) iter.Seq2[githost.Commit, error] {
	...
	commitErr := f.commitErr[repo.FullName()]
	return func(yield func(githost.Commit, error) bool) {
		if commitErr != nil {
			yield(githost.Commit{}, commitErr)
			return
		}
		if f.measureConc {
...
// skipped before any commit/PR fetch and counted in
// stats.ReposSkippedArchived — so a workspace of 2000 mostly-archived
// repos doesn't burn API budget on read-only history.
func TestIngestGitHost_SkipsArchivedRepos(t *testing.T) {
	...
	stats, err := IngestGitHost(context.Background(), s, fake, nil, GitHostFilter{})
	if err != nil {
		t.Fatalf("ingest: %v", err)
	}
	...
	if stats.ReposSkippedArchived != 2 {
		t.Errorf("stats.ReposSkippedArchived = %d, want 2", stats.ReposSkippedArchived)
	}
	if stats.Commits != 1 {
...
// TestIngestGitHost_PerRepoFailureIsResumable simulates the 24h-into-
// baseline failure: one repo in the middle of the workspace errors out
// (here a synthetic 301-equivalent). The run must:
...
//     them),
//   - return an error wrapping ErrPartialFailure so the caller knows the
//     run was partial,
//   - report the failure to the progress reporter for the operator.
func TestIngestGitHost_PerRepoFailureIsResumable(t *testing.T) {
	...
	stats, err := IngestGitHost(context.Background(), s, fake, progress.Plain(&buf), GitHostFilter{})
	if err == nil {
		t.Fatal("expected non-nil error for partial-failure run")
	}
	if !errors.Is(err, ErrPartialFailure) {
		t.Errorf("error %q does not wrap ErrPartialFailure", err)
	}
	...
	if out := buf.String(); !contains(out, "acme/broken: FAILED") {
