...
const (
	...
	// most one hour out, so anything longer is almost certainly clock
...
// rateLimitLog is where the retry/redirect wrappers write their one-line
// "sleeping until reset" / "redirected" notes. Stored as a package-level
...
// --- request layer: rate-limit retries, redirects, transient retries ---

// doRequest wraps c.api.Do with a GitHub-specific rate-limit retry loop.
//
// GitHub signals rate limiting two ways:
//   - Primary REST limit: HTTP 403 with X-RateLimit-Remaining: 0 and an
//     X-RateLimit-Reset unix-epoch header. The shared apiclient surfaces
//     this as a fatal *StatusError because 403 isn't in its generic
//     429/5xx retry set.
//   - Secondary (abuse) limit: HTTP 429, sometimes with Retry-After.
//     apiclient retries those with bounded backoff and gives up on
//     Retry-After hints past its 60s cap; the longer waits (up to an
//     hour) are handled here.
//
...
func (c *Client) doRequest(ctx context.Context, method, path, rawQuery string, body io.Reader) (*http.Response, error) {
	...
	for attempt := 0; attempt <= maxRateLimitRetries; attempt++ {
		...
		_, _ = fmt.Fprintf(rateLimitLog, "github: rate limit hit, sleeping %s until %s (attempt %d/%d)\n",
	...
	if lastErr == nil {
		// Unreachable — the loop only completes via the retry path, which
		// always sets lastErr. The guard is for nilaway's flow analysis
		// (and callers must never see (nil, nil)).
		lastErr = fmt.Errorf("github %s %s: rate limit retries exhausted", method, path)
	}
	return nil, lastErr
}

// doOnceFollowingRedirects issues the request and, when the upstream
// answers with a redirect pointing at the same host, re-issues against the
// new path. GitHub returns 301 when a repo has been renamed or
// transferred, pointing at the stable /repositories/{id}/... path; the
...
func (c *Client) doOnceFollowingRedirects(ctx context.Context, method, path, rawQuery string, body io.Reader) (*http.Response, error) {
	for hop := 0; hop <= maxRedirects; hop++ {
		...
		_, _ = fmt.Fprintf(rateLimitLog, "github: %s %s redirected → %s\n", method, path, newPath)
...
// Returns (path, rawQuery, true) when err is a redirect with a usable
...
func (c *Client) getPage(ctx context.Context, path, rawQuery string, dest any) (string, error) {
	...
	for attempt := 0; attempt <= maxTransientRetries; attempt++ {
		...
		_, _ = fmt.Fprintf(rateLimitLog, "github: transient error, sleeping %s before retry (attempt %d/%d): %v\n",
...
func (c *Client) getPageOnce(ctx context.Context, path, rawQuery string, dest any) (nextURL string, _ error) {
